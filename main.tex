...
\todo{Daniel: rewrote. Jonas: the above sentence is unnecessarily complicated}
...
\todo{Daniel: rewrote}
\todo{Jonas: I think it might help to add just one sentence clarifying that if the monitor is sound and it executes all scripts then the technique is also sound. I realize that this is what the sentence say, but it is not immediately clear}
...
\todo{Daniel: unsure - discuss on monday?}
\todo{Jonas: reporting to the user and letting the user decide is equally possible for the other approaches as well. Perhaps it is less likely in the integrator, which is more likely to make the decision for the user, or the proxy in a corporate setting, which is likely to report back to a central unit. The main benefit is user control, but perhaps more focus on the policies. What do you think?}
...
\todo{Daniel: the only way to handle write is to look at the string and try to interpret what the intention was, i.e., if we are writing a script into, the we add a script node. We can't run document.write, we cant access the parser, how are we supposed to be able to implement document.write?}
\todo{Jonas: can we construct a simple example which cannot be properly reproduced by the monitor? It might help to understand the problem.}
...
a loading page.
\todo{Daniel: fixed}
\todo{Jonas: I seem to remember that they changed the behaviour of innerHTML to not execute scripts when assigning innerHTML. It is still possible to add nodes with event handlers though that can be triggered by user interaction or the script that modified the innerHTML, and not by, e.g. autofocus. Not sure what I'm trying to say really, just that someone might argue that this has been dealt with, even though it's not.}

% data:text/html,<html><head><script>function change(){document.getElementById('change').innerHTML='Changed<input id="focus" onfocus="alert(/changed/)"><script>function delayed(){alert("delayed change")}<'+'/script>';try{delayed=function(){alert('no delay')};setTimeout(delayed,20000);}catch(e){alert(e)}document.getElementById('focus').focus()}onload=change</script></head><body><div id="change">Change me</div><input onfocus="alert('change me')" autofocus></body></html>
...
\todo{Daniel: not really, but rephrased }
\todo{Jonas: missing a "not"? "the extension is not provided with the content"}
...
When loaded the extension first turns off the standard JavaScritp engine by
