...
Whether the requirements of transparency can be met
...
any other site (\emph{server-side checkers}); and (ii) disallow the password
to leave the browser (\emph{client-side checkers}). 
Clearly, the first type places trust on the service provide not to
abuse the password, while the second type does not require such trust,
in line with the principle of least
...
One way to enforce the second type of policy, that the password does not leave the browser, is to
isolate the service, i.e., prevent it from performing any communication.
While effective, such a stern policy risks breaking the functionality of the service.
It is common that pages employ some for of usage statistics tracking such as Google
Analytics. Google Analytics requires that usage information is allowed to be gathered and
sent to Google for aggregation. Using information-flow tracking we can guarantee
that the password is not sent to the service provider or Google without disallowing
communication in general.


The sites that fall into the first category include 
\url{https://testalosenord.pts.se/}, provided by the Swedish Post
and Telecom Authority, as well as 
\todo{Add list}.
...
Of the different service we have selected two that we present in greater
detail. 
%
