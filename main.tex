...
monitored context; an effort that must continue as the monitor executes. 
...
executed upon interpretation. Failing to address this problem, the script code will execute 
...
Before the response is delivered to the client, it is 
rewritten. Each content-type related to JavaScript is 
...
%Jonas
%xperimental setup (machine, processor, OS,,,,)
What we measure and on what pages
Results in a form of a table (8 numbers)

We measure the average overhead introduced by three 
of the four architectures. The overhead of the fourth architecture would depend 
entirely on its implementation and usage and can therefore not be generally measured.

The tests were performed in Firefox under the Windows 7 64 bit Service Pack 1 operating system on a machine running on an Intel Core i7-3250M 2.9 GHz CPU, with 8 GB of memory.

\begin{table}[h!]
\center
\begin{tabular}{l|c|c|c|c}
& Reference & Browser Extension & Web proxy & Suffix proxy \\ \hline
Via HTTP & Yes & Yes & Yes \\ \hline
Via HTTPS & Yes & Yes & No \\ \hline
\end{tabular}
